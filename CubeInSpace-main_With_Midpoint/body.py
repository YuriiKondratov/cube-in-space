...
class RigidBody:
    ...
    def eilers_step(self, dt):
        ...
        #self.centre_of_mas_pos = self.centre_of_mas_pos + self.linear_momentum / self.mass * dt
        #y i+1 = y i + f(xi, yi)*h
    ...
    def midpoint_step(self, dt):
        # full body state update
        self.centre_of_mas_pos += dt * ((self.linear_momentum + self.total_force * dt/2) / self.mass)
        #y i+1 = y i + f(xi + h/2, yi + h/2*f(xi, yi))*h
        self.rotation_quaternion += Quaternion([0, *self.angular_velocity]) * self.rotation_quaternion * 0.5 * dt
        self.linear_momentum += (self.total_force + dt/2)* dt
        self.angular_momentum += (self.torque + dt/2) * dt
